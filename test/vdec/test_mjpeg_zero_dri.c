...
static int check_copy(
    const uint8_t *input, size_t input_size, const uint8_t *expected, size_t expected_size, int in_place
) {
    uint8_t output[128];
    size_t output_size = 0;

    CHECK(input_size <= sizeof(output));
    if (in_place) {
        memcpy(output, input, input_size);
        CHECK(linlon_mjpeg_remove_zero_dri(output, input_size, output, sizeof(output), &output_size) == 0);
    } else {
        CHECK(linlon_mjpeg_remove_zero_dri(input, input_size, output, sizeof(output), &output_size) == 0);
    }
...
int main(void) {
    ...
    CHECK(check_copy(zero_dri, sizeof(zero_dri), zero_dri_expected, sizeof(zero_dri_expected), 0) == 0);
    CHECK(check_copy(zero_dri, sizeof(zero_dri), zero_dri_expected, sizeof(zero_dri_expected), 1) == 0);
    CHECK(check_copy(nonzero_dri, sizeof(nonzero_dri), nonzero_dri, sizeof(nonzero_dri), 0) == 0);
    CHECK(check_copy(app_contains_pattern, sizeof(app_contains_pattern), app_contains_pattern,
              sizeof(app_contains_pattern), 0) == 0);
    CHECK(check_copy(multiple_zero_dri, sizeof(multiple_zero_dri), multiple_expected,
              sizeof(multiple_expected), 0) == 0);
    CHECK(check_copy(truncated, sizeof(truncated), truncated, sizeof(truncated), 0) == 0);
    CHECK(check_copy(nonstandard_dri_length, sizeof(nonstandard_dri_length), nonstandard_dri_length,
              sizeof(nonstandard_dri_length), 0) == 0);
    CHECK(check_copy(no_soi, sizeof(no_soi), no_soi, sizeof(no_soi), 0) == 0);
    CHECK(linlon_mjpeg_remove_zero_dri(
              zero_dri, sizeof(zero_dri), too_small, sizeof(too_small), &output_size) != 0);
